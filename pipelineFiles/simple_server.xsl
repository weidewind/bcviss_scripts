...
<xsl:template match="/*">
  ...
  <table style="font-family:calibri" cellpadding="5" border="1" cellspacing="0">
	...
	<xsl:for-each select="chromatogram">
		...
		<td  colspan="7"><xsl:text>No valid 16S sequences with acceptable BLAST results were produced</xsl:text></td>
		</tr>
		</xsl:when>
		<xsl:otherwise>
		...
		<tr class="collapsing-panel">
			<td colspan = "8">
				...
				<a href="{@fasta_ref}"><xsl:text> All sequences in FASTA format</xsl:text> </a> <br></br>
				<br></br>
				...
				<xsl:sort order="descending" select="@expectation"/>
					...
					<a href="{@img_ref}"><xsl:text> Phylogenetic tree (SVG format)</xsl:text> </a> <br></br>
					<a href="{@newick_ref}"><xsl:text> Phylogenetic tree (Newick format)</xsl:text></a><br></br>
					<br></br>
